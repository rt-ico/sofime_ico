...
//                    model.sort.asc = path[1] === "asc" ? true : path[1] === "desc" ? false : null;
				...
				Promise.all([
					...
					Promise.all([
						RT.jQuery.get({
							url: home.Route.sfAutoMailOutbox + "sent/" + idParam(model.filter.expat) + '/' +idParam(model.filter.staff),
							contentType: false,
							dataType: "json"
						}).then(function (rs) {
					...
					home.View.documentTitle = txt.navMailSent;
					...
					$items.on('click', function (evt) {
						...
						home.Router.go(["sfMailItemSent", itemId]);
					});
